...
<?php
	...
	is_plugin_active( 'translatepress-multilingual/index.php' )) {
		...
		if (function_exists('pll_languages_list')) {
			if (null != pll_languages_list(array('fields' => array()))) {
				foreach (pll_languages_list(array('fields' => array())) as $lang) {
		...
		if (isset($translatepress_settings['publish-languages'])) {
			foreach ($translatepress_settings['publish-languages'] as $tp_language) {
				$tp_language = substr($tp_language, 0, 2);
				if (isset($tp_language)) {
					$languages[$tp_language] = $tp_language;
				}
			}	
		}
	...
	if (isset($_GET['act'])) {
		...
		<div class="notice dsgvodeleteall is-dismissible notice-warning" data-notice="dsgvodeleteall">
			<p><?php echo wp_kses(__("Are you sure you want to delete the plugin including all saved settings?", "dsgvo-all-in-one-for-wp"), array('a' => array( 'href' => array(), 'title' => array()), 'br' => array(), 'em' => array(), 'strong' => array())); ?></p>
			<p><?php echo wp_kses(__("The settings cannot be restored!", "dsgvo-all-in-one-for-wp"), array('a' => array( 'href' => array(), 'title' => array()), 'br' => array(), 'em' => array(), 'strong' => array())); ?></p>
			<a href="" class="button button-primary"><?php echo wp_kses(__("Delete plugin including all settings", "dsgvo-all-in-one-for-wp"), array('a' => array( 'href' => array(), 'title' => array()), 'br' => array(), 'em' => array(), 'strong' => array())); ?></a>
		</div>
		<?php
		}
	}
	...
	if(isset($_POST["submit"])){ 
		...
		if ($privilegs == 1) {
			...
			if (isset($_POST["ga_consent_mode"])) { $ga_consent_mode = sanitize_text_field($_POST["ga_consent_mode"]);	} else { $ga_consent_mode = ""; }
			if (isset($_POST["ga_consent_modegtag"])) { $ga_consent_modegtag = sanitize_text_field($_POST["ga_consent_modegtag"]);	} else { $ga_consent_modegtag = ""; }
			...
			if ($showpolylangoptions == true && array_key_exists("it", $languages) or $language == "it") {
			...
			if ($showpolylangoptions == true && array_key_exists("en", $languages) or $language == "en") {
			...
			if (isset($_POST["ga_consent_mode"])) { update_option("dsdvo_ga_consent_mode", sanitize_text_field($_POST["ga_consent_mode"]), false);	} else { update_option("dsdvo_ga_consent_mode", "", false); }
			if (isset($_POST["ga_consent_modegtag"])) { update_option("dsdvo_ga_consent_modegtag", sanitize_text_field($_POST["ga_consent_modegtag"]), false);	} else { update_option("dsdvo_ga_consent_modegtag", "", false); }
			...
			if ($ga_consent_mode == "on") {
				$ga_consent_mode = "checked='checked'";
			} else {
				$ga_consent_mode = "";
			}				
			
			if ($ga_consent_modegtag == "on") {
				$ga_consent_modegtag = "checked='checked'";
			} else {
				$ga_consent_modegtag = "";
			}							
			...
			$saved_policy_text_1 = get_option("dsdvo_policy_text_1");

			update_option("dsdvo_policy_update_date", $update_date, false);
			...
			echo '<div class="notice notice-success" class="updated fade"><p>'.wp_kses(__('Settings saved successfully', 'dsgvo-all-in-one-for-wp'), array('a' => array( 'href' => array(), 'title' => array()), 'br' => array(), 'em' => array(), 'strong' => array())).'<span class="dashicons dashicons-yes"></span></p></div>';
			...
			$ga_consent_mode = sanitize_text_field(get_option("dsdvo_ga_consent_mode"));
			$ga_consent_modegtag = sanitize_text_field(get_option("dsdvo_ga_consent_modegtag"));
 ...
 ?>
	...
	<img src="<?php echo wp_kses(plugins_url( '../assets/img/dsgvo_free_logo.png', dirname(__FILE__) ), $kses_allowed_html); ?>" class="dsgvoaio_backendlogo"/>
	<h1 class="dsgvoaio_main_h1">&nbsp;</h1>
	<div id="dsdvo_left">

 
	<p style="display: none;font-size: 14px;"><?php echo wp_kses(__("Here you can set up everything about DSGVO All in One.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></p>	<p style="font-size: 14px;"><?php echo wp_kses(__("<b>Important:</b> Please take a short time and go through <u>all</u> fields.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></p>
 	<p style="font-size: 14px;"><?php echo wp_kses(__("It is important to set everything correctly because only then the plugin can do good job.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></p>
	<p style="font-size: 14px;"><?php echo wp_kses(__("<b>Info:</b> The <a href='http://dsgvo-for-wp.com/#shop' target='blank'>PRO Version</a> offers many advantages including premium support and updates.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></p>	 
	<p style="font-size: 14px;"><?php echo wp_kses(__("<b>Need Help?</b> Email to michaelleithold18@gmail.com or via the <a href='https://wordpress.org/support/plugin/dsgvo-all-in-one-for-wp/' target='blank'>WordPress Forum</a>.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></p>	 
	<p style="font-size: 14px;"><?php echo wp_kses(__("<b>Cache:</b> Always clear the cache after each change (browser cache and plugin cache if necessary).", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></p>
	...
	<br />
		<form method="post" action="admin.php?page=dsgvoaio-free-settings-page">
			...
			<?php } ?>				
				...
				<div class="options" id="dsgvooptions">
					<h2 class="dsgvoheader"><a><span class="dashicons dashicons-admin-users"></span><?php echo wp_kses(__("Data of the Owner", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?> <span style="font-size:13px">(<?php echo wp_kses(__("Address data, Contact details", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>)
					<?php if (empty($dsgvoaioperson) or empty($dsgvoaiocity)) { ?>
					<span class="infonodata">[<b><?php echo wp_kses(__("Attention!", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></b>&nbsp;<?php echo wp_kses(__("no data set.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>]</span>	
					<?php } ?>
					...
					<p><b class="blabel"><?php echo wp_kses(__("Company name", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					...
					<p><b class="blabel"><?php echo wp_kses(__("Responsible person", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input dsgvoaioperson" type="text" name="dsgvoaioperson" value="<?php echo esc_html($dsgvoaioperson); ?>"/></p>
					<p><b class="blabel"><?php echo wp_kses(__("Street & House number", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input dsgvoaiostreet" type="text" name="dsgvoaiostreet" value="<?php echo esc_html($dsgvoaiostreet); ?>"/></p>
					<p><b class="blabel"><?php echo wp_kses(__("Zip code", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input dsgvoaiozip" type="text" name="dsgvoaiozip" value="<?php echo esc_html($dsgvoaiozip); ?>"/></p>
					<p><b class="blabel"><?php echo wp_kses(__("City", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input dsgvoaiocity" type="text" name="dsgvoaiocity" value="<?php echo esc_html($dsgvoaiocity); ?>"/></p>
					<p><b class="blabel"><?php echo wp_kses(__("Country", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input dsgvoaiocountry" type="text" name="dsgvoaiocountry" value="<?php echo esc_html($dsgvoaiocountry); ?>"/></p>
					<p><b class="blabel"><?php echo wp_kses(__("Phone", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input dsgvoaiophone" type="text" name="dsgvoaiophone" value="<?php echo esc_html($dsgvoaiophone); ?>"/></p>
					<p><b class="blabel"><?php echo wp_kses(__("Fax", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input dsgvoaiofax" type="text" name="dsgvoaiofax" value="<?php echo esc_html($dsgvoaiofax); ?>"/></p>					
					<p><b class="blabel"><?php echo wp_kses(__("E-Mail", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input dsgvoaiomail" type="text" name="dsgvoaiomail" value="<?php echo wp_kses_post($dsgvoaiomail); ?>"/></p>
					<p><b class="blabel"><?php echo wp_kses(__("Vat-ID", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input dsgvoaiousdid" type="text" name="dsgvoaiousdid" value="<?php echo wp_kses(__("Is defined under Imprint!", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" readonly/></p>
					<p><span class="dashicons dashicons-info"></span>&nbsp;<?php echo wp_kses(__("The data defined here are used for the privacy policy and for the imprint.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<?php echo wp_kses(__("It is very important that you also define all necessary data under the point Imprint", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.</p>
					</div>		
					...
					<p class="dsdvo_options">
						<span><a href="#" class="services_content" data-tab="analytic"><?php echo wp_kses(__("Visitor counter services / Analytics / Counter", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>&nbsp;<span class="toggle-indicator" aria-hidden="true"></span></a></span>
					</p>					
					
					<span class="content_analytic dsgvoaio_hide">
					<p class="dsdvo_options">
					<b><?php echo wp_kses(__("Facebook Pixel", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input use_fbpixel" type="checkbox" name="use_fbpixel" <?php echo esc_html($use_fbpixel); ?>/><br />
					<label><?php echo wp_kses(__("Do you use Facebook Pixel?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					<br />
					<br />
					<span class="fbpixelwrap">					
					<b><?php echo wp_kses(__("Facebook Pixel ID", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
					<input class="dsdvo_input" type="text" name="fbpixelid" value="<?php echo esc_html($fbpixelid); ?>" /><br />
					<label>
					<?php echo wp_kses(__("Enter your Facebook Pixel ID here.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<?php echo wp_kses(__("Format: 244123839385278", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<br />
					<?php echo wp_kses(__("<u>Important:</u> Be sure to deactivate the event setup on Facebook Pixel!", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<?php echo wp_kses(__("For more information see here: <a href='https://dsgvo-for-wp.com/dokumentation/' target='blank'>Plugin Documentation</a>", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					...
					<div class="dsdvo_options">
					<b><?php echo wp_kses(__("Google Analytics", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input use_ga" type="checkbox" name="use_ga" <?php echo esc_html($use_ga); ?>/><br />
					<label><?php echo wp_kses(__("Do you use Google Analytics?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					<br />
					<br />
					<span class="gawrap">
						<span class="dsgvoaio_ga_options">
							...
							<label class="dsgvoaio_bold" for="checkbox-1"><?php echo wp_kses(__("Manual integration (Without Plugin)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							...
							<label class="dsgvoaio_bold" for="checkbox-1"><?php echo wp_kses(__("MonsterInsights (WordPress Plugin)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>		
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							...
							<label class="dsgvoaio_bold" for="checkbox-1"><?php echo wp_kses(__("Analytify (WordPress Plugin)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>								
						</span>
					<?php
					$ga_type = get_option('dsdvo_ga_type', 'manual');
					?>
					<span class="dsgvoaio_ga_type_manual">
					<b><?php echo wp_kses(__("Google Analytics ID", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
					<input class="dsdvo_input" type="text" name="gaid" placeholder="<?php echo wp_kses(__("Enter you GA ID here...", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" value="<?php echo esc_html($gaid); ?>" /><br />
					<label>
					<?php echo wp_kses(__("Enter your Google Analytics ID here.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<?php echo wp_kses(__("Format: UA-XXXXXXXXX-X / G-XXXXXXXXXXX", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					</label>
					<br />
					<b><?php echo wp_kses(__("Google Consent Mode V2", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input ga_consent_mode" type="checkbox" name="ga_consent_mode" <?php echo esc_html($ga_consent_mode); ?>/><br />
					<label><?php echo wp_kses(__("Would you like to use <a href=\"https://developers.google.com/tag-platform/security/guides/consent?hl=de&consentmode=basic\" target=\"blank\">Google Consent Mode v2</a>?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					</label>
					<br/>					
					<br />
					<b><?php echo wp_kses(__("Standard OptIn / OptOut", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						...
						foreach ($ga_optinoutarr as $key => $ga_optinout) {
							...
							echo "<option value='".wp_kses($key, $kses_allowed_html)."' ".wp_kses($s, $kses_allowed_html).">".esc_html($ga_optinout)."</option>";
						}
						?>
						</select>	
						<br />
						<?php echo wp_kses(__("Determine whether Google Analytics should be accepted on the first visit to the site without the user's consent (OptOut).", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
						</span>
						...
						<?php if ( is_plugin_active( 'google-analytics-for-wordpress/googleanalytics.php' ) or is_plugin_active( 'google-analytics-premium/googleanalytics-premium.php' ) ) { ?>
								<span style="color: #46b450"><?php echo wp_kses(__("The plugin MonsterInsights was found", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><span class="dashicons dashicons-yes"></span></span><br/>
								<?php echo wp_kses(__("If you activate this option you can use MonsterInsights GDPR compliant (2 click solution).", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
								<?php echo wp_kses(__("All settings are taken over by MonsterInsights. There is nothing more to do than to activate this option.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>								
						<?php } else { ?>
								<span style="color: red"><?php echo wp_kses(__("The Google Analytics Dashboard Plugin Plugin for WordPress by MonsterInsights was not found.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><span class="dashicons dashicons-no-alt"></span></span><br />
								<?php echo wp_kses(__("Please make sure that the plugin is installed and activated before you activate this option and save the settings!", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>		
						<?php }	?>
						</span>
						<span class="dsgvoaio_ga_type_analytify">
						<?php if ( is_plugin_active( 'wp-analytify/wp-analytify.php' ) ) { ?>
								<span style="color: #46b450"><?php echo wp_kses(__("The plugin Analytify was found", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><span class="dashicons dashicons-yes"></span></span><br/>
								<?php echo wp_kses(__("If you activate this option you can use Analytify GDPR compliant (2 click solution).", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
								<?php echo wp_kses(__("All settings are taken over by MonsterInsights. There is nothing more to do than to activate this option.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>									
						<?php } else { ?>
								<span style="color: red"><?php echo wp_kses(__("The Google Analytics Dashboard Plugin Plugin for WordPress by Analytify was not found.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><span class="dashicons dashicons-no-alt"></span></span><br />
								<?php echo wp_kses(__("Please make sure that the plugin is installed and activated before you activate this option and save the settings!", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>		
						<?php }	?>
						</span>						
						<?php //} ?>
					...
					<p class="dsdvo_options">
					<b><?php echo wp_kses(__("Google Tag Manager", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input use_gtagmanager" type="checkbox" name="use_gtagmanager" <?php echo esc_html($use_gtagmanager); ?>/><br />
					<label><?php echo wp_kses(__("Do you use Google Tag Manager?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					<br />
					<br />
					<span class="gtagmanagerwrap">
					<b><?php echo wp_kses(__("Google Tag Manager ID", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
					<input class="dsdvo_input gtagmanagerid" type="text" name="gtagmanagerid" value="<?php echo esc_html($gtagmanagerid); ?>" /><br />
					<label>
					<?php echo wp_kses(__("Enter your Google Tag Manager ID here.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<?php echo wp_kses(__("Format: GTM-XXXXXXX", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					</label>
					<br />
					<b><?php echo wp_kses(__("Google Consent Mode V2", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input ga_consent_modegtag" type="checkbox" name="ga_consent_modegtag" <?php echo esc_html($ga_consent_modegtag); ?>/><br />
					<label><?php echo wp_kses(__("Would you like to use <a href=\"https://developers.google.com/tag-platform/security/guides/consent?hl=de&consentmode=basic\" target=\"blank\">Google Consent Mode v2</a>?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					</label>
					<br/>					
					<br />					
					<b><?php echo wp_kses(__("Replace integration automatically [Beta]", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input remove_gtagmanager" type="checkbox" name="remove_gtagmanager" <?php echo esc_html($remove_gtagmanager); ?>/><br />
					<label><?php echo wp_kses(__("Should ALL existing Google Tag Manager integrations be replaced according to GDPR (2 click solution)?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<?php echo wp_kses(__("All existing integrations of the domain googletagmanager.com are automatically replaced.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
					</label>
					<br />							
					<br />	
					</span>
					...
					<b><?php echo wp_kses(__("VG Wort Pixel", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
					<input  class="dsdvo_input use_vgwort" type="checkbox" name="use_vgwort" <?php echo esc_html($use_vgwort); ?>/><br />
					<label><?php echo wp_kses(__("Do yo use VG Wort?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					<br />
					<br />
					<span class="vgwortwrap">
					<?php echo wp_kses(__("If you use the services of VG Wort, activate this option so that the corresponding policy text is inserted in the privacy policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
					<b><?php echo wp_kses(__("Important", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;<?php echo wp_kses(__("You have to adapt the text of the privacy policy! ([PLEASE CHECK if this is the case with your publisher!])", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
					<br />
					<br />
					<b><?php echo wp_kses(__('Shortcode for embedding VG Wort:<br />', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></b><br />
					...
					<?php echo wp_kses(__('With the shortcode you can integrate VG Wort GDPR compliant on any desired page.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<br />
					<b><?php echo wp_kses(__("Standard OptIn / OptOut", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						...
						foreach ($vgwort_optinoutarr as $key => $vgwort_optinout) {
							...
							echo "<option value='".wp_kses($key, $kses_allowed_html)."' ".wp_kses($s, $kses_allowed_html).">".esc_html($vgwort_optinout)."</option>";
						}
						?>
						</select>	
						<br />
						<?php echo wp_kses(__("Determine if VG Wort should be accepted on the first visit to the site without the user's consent (OptOut).", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
						<?php echo wp_kses(__("We do not recommend allowing the service without the user's consent as this is not compliant in our opinion!", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<br />
					<b><?php echo wp_kses(__("Replace integrations automatically", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input remove_vgwort" type="checkbox" name="remove_vgwort" <?php echo esc_html($remove_vgwort); ?>/><br />
					<label><?php echo wp_kses(__("Should ALL existing integrations of VG Wort GDPR compliant replaced (2 click solution)?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<?php echo wp_kses(__("All existing integrations of the domain vgwort.de are automatically replaced.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<?php echo wp_kses(__("The plugin \"Worthy\" is supported.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
					</label>
					<br />							
					<br />					
					...
					<span class="dsdvo_options">
					<b><?php echo wp_kses(__("Koko Analytics", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
					<input  class="dsdvo_input use_koko" type="checkbox" name="use_koko" <?php echo esc_html($use_koko); ?>/><br />
					<label><?php echo wp_kses(__("Do you use Koko Analytics?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					<br />
					<br />
					<span class="kokowrap">
					<?php echo wp_kses(__("If you use Koko Analytics activate this option so that the corresponding privacy text is inserted in the privacy policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
					<br />
					<b><?php echo wp_kses(__("Standard OptIn / OptOut", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<select  class="dsdvo_input"  name="koko_optinoutsetting">
						<?php
						$koko_optinoutarr = array('optin' => wp_kses(__('OptOut (need consent)', 'dsgvo-all-in-one-for-wp'), $kses_allowed_html), 'optout' => wp_kses(__('OptIn (does not require consent)', 'dsgvo-all-in-one-for-wp'), $kses_allowed_html));
						foreach ($koko_optinoutarr as $key => $koko_optinout) {
							...
							echo "<option value='".wp_kses($key, $kses_allowed_html)."' ".wp_kses($s, $kses_allowed_html).">".esc_html($koko_optinout)."</option>";
						}
						?>
						</select>	
						<br />
						<?php echo wp_kses(__("Determine if Koko Analytics should be accepted on the first visit to the site without user consent (OptOut).", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					...
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Piwik/Matomo", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
						<input  class="dsdvo_input" type="checkbox" name="use_piwik" <?php echo esc_html($use_piwik); ?>/><br />
						<label><?php echo wp_kses(__("If you want to use Piwik/Matomo on your site check this option and enter all required details", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.</label><br />
						<br />
					<span class="piwikwrap">
					
						<b><?php echo wp_kses(__("Host URL", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>  <br />
						...
						<label>
						<?php echo wp_kses(__("Enter the host URL of Matomo here (setTrackerUrl)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
						<b><?php echo wp_kses(__("Format", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;//foo.bar/ &nbsp;
						(<?php echo wp_kses(__("the Trailing Slash / at the end is important - yes really without http/https/www and without matomo.php at the end", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>!)
						</label><br />

						<br />
						<b><?php echo wp_kses(__("Tracker Endpoint", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>  <br />
						...
						<label>
						<?php echo wp_kses(__("Select a Endpoint.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
						</label>						
						<br />
						
						<b><?php echo wp_kses(__("Site ID", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>  <br />
						<input type="number"  class="dsdvo_input piwik_siteid" name="piwik_siteid" value="<?php echo esc_html($piwik_siteid); ?>"/><br />
						<label>
						<?php echo wp_kses(__("Enter here the Site ID of Matomo (setSiteId)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
						</label>
						<br/>						
						<b><?php echo wp_kses(__("Important", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>					
						<?php echo wp_kses(__("If you use the plugin from Matomo for Wordpress please enter all necessary data here and set in the Matomo plugin settings the setting \"Add tracking code\" to the option \"Disabled\".", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
					...
					<p class="dsdvo_options">
						<span><a href="#" class="services_content" data-tab="social"><?php echo wp_kses(__("Social Networks / Sharing", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>&nbsp;<span class="toggle-indicator" aria-hidden="true"></span></a></span>
					</p>					
					...
					<b><?php echo wp_kses(__("Shareaholic", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
					<input  class="dsdvo_input use_shareaholic" type="checkbox" name="use_shareaholic" <?php echo esc_html($use_shareaholic); ?>/><br />
					<label><?php echo wp_kses(__("Do you use Shareaholic?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					...
					<label><?php echo wp_kses(__("Enter the Shareaholic site ID here (SiteId)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.
					</label>
					<br />
					<br />				
					...
					<br />
					<p class="dsdvo_options">
					<b><?php echo wp_kses(__("Facebook Comments", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					...
					<label><?php echo wp_kses(__("Do you use Facebook Comments?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					<br />
					<br />
					...
					<br />
					<p class="dsdvo_options">
					<b><?php echo wp_kses(__("Facebook (Like Button)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input use_facebooklike" type="checkbox" name="use_facebooklike" <?php echo esc_html($use_facebooklike); ?>/><br />
					<label><?php echo wp_kses(__("Do you use Facebook Like Button?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					<br />
					<br />
					...
					<b><?php echo wp_kses(__("Username", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><br />
					<input class="dsdvo_input" type="text" name="twitterusername" class="twitterusername" value="<?php echo esc_html($twitterusername); ?>"/><br />
					<br />
					<b><?php echo wp_kses(__('Shortcode for embedding the Tweet Button', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></b><br />
					<input class="dsdvo_input twittershortcode" type="text" name="twittershortcode" value='[dsgvo_twitter_button]' readonly/><br />
					<?php echo wp_kses(__('With the shortcode you can integrate the Tweet Button GDPR compliant on every desired page.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					</span>
					</p>
					...
					<b><?php echo wp_kses(__("AddThis", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input use_addthis" type="checkbox" name="use_addthis" <?php echo esc_html($use_addthis); ?>/><br />
					<label><?php echo wp_kses(__("Do you use AddThis?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					<br /> 					<br />
					<span class="addthiswrap">
					<b><?php echo wp_kses(__("Your PUB ID", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><br />
					<input class="dsdvo_input" type="text" name="addthisid" class="addthisid" value="<?php echo esc_html($addthisid); ?>"/><br />
					<br />
					<b><?php echo wp_kses(__('Shortcode for including the AddThis Button:<br />', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></b>
					<input class="dsdvo_input addthisshortcode" type="text" name="twittershortcode" value='[dsgvo_addthis]' readonly/><br />
					<?php echo wp_kses(__('With the shortcode you can integrate the AddThis Button GDPR compliant on every desired page.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					</span>
					</p>
					<br />
					<p class="dsdvo_options">
					<b><?php echo wp_kses(__("Linkedin", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input use_linkedin" type="checkbox" name="use_linkedin" <?php echo esc_html($use_linkedin); ?>/><br />
					<label><?php echo wp_kses(__("Do you use Linkedin?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					<br />
					<br />
					...
					<p class="dsdvo_options">
						<span><a href="#" class="services_content" data-tab="video"><?php echo wp_kses(__("Video / Audio Services", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>&nbsp;<span class="toggle-indicator" aria-hidden="true"></span></a></span>
					</p>					
					
					<span class="content_video dsgvoaio_hide">	
					<p class="dsdvo_options">
					
					<b><?php echo wp_kses(__("Vimeo", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
					<input  class="dsdvo_input use_vimeo" type="checkbox" name="use_vimeo" <?php echo esc_html($use_vimeo); ?>/><br />
					<label><?php echo wp_kses(__("Do you use Vimeo Videos?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					
					<br /> 
					<br />					
					<span class="vimeowrap">
					<b><?php echo wp_kses(__('Shortcode for embedding Vimeo Videos', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><br />
					...
					<?php echo wp_kses(__('With the shortcode you can integrate Vimeo Videos GDPR/DSGVO compliant on any desired page', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
					<?php echo wp_kses(__('Additional parameters: width="width" height="height" thumbnail="true|false|url"', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<br />
					<b><?php echo wp_kses(__('Replace existing embeddings automatically GDPR/DSGVO compliant', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><br />					
					<?php echo wp_kses(__('The PRO Version offers the feature to automatically replace all existing vimeo integrations', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />					
					</span>			
					</p>
					<br />					
					<p class="dsdvo_options">
					
					<b><?php echo wp_kses(__("Youtube", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
					<input  class="dsdvo_input use_youtube" type="checkbox" name="use_youtube" <?php echo esc_html($use_youtube); ?>/><br />
					<label><?php echo wp_kses(__("Do you use Youtube?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					
					<br /> 
					<br />					
					<span class="youtubewrap">
					<b><?php echo wp_kses(__('Shortcode for embedding Youtube Videos', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><br />
					...
					<?php echo wp_kses(__('With the shortcode you can integrate Youtube Videos GDPR/DSGVO compliant on any desired page', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
					<?php echo wp_kses(__('Additional parameters: width="width" height="height" autoplay="0|1" rel="0|1" thumbnail="true|false|url"', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<br />	
					<b><?php echo wp_kses(__('Replace existing embeddings automatically GDPR/DSGVO compliant', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><br />					
					<?php echo wp_kses(__('The PRO Version offers the feature to automatically replace all existing youtube integrations', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />					
					</span>			
					</p>
					</span>
					...
					<b><?php echo wp_kses(__("Integrate more external services GDPR compliant such as:", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></b><br />
					<br />
					<?php echo wp_kses(__("eTracker, Statcounter, Clicky, Google Adsense, Amazon Ads, Google Maps, Komoot, OpenStreetMap + LeafLetMap, Youtube & Youtube Playlist, Vimeo, Dailymotion, SoundCloud, HearThis, MixCloud, Disqus, Instagram Feed, Facebook Like/Share/Teilen – Kommentare, Facebook Pixel, Printerest, ShareThis, Shareaholic, AddThis, AddToAny, SlideShare, reCAPTCHA, OneSignal, Slimstats + iFrame / Cotent Blocker + 5 zusätzliche Dienste die per JS Code ausgeführt werden bequem über die Einstellungen DSGVO konform nutzen,", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<br />
					<b><?php echo wp_kses(__("...in the Pro Version.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></b>
					</p>					
				
					</span>
					<br />
					<h2 class="dsgvoheader"><a><span class="dashicons dashicons-desktop"></span><?php echo wp_kses(__("Settings Cookie Notice <span style='font-size:13px'>(Cookie Notice, Design/Style, Buttons, Lifetime, Texts)</span>", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></a></h2>
					<div class="dsgvooptionsinner">					
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Cookie Notice/Message", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
						<input  class="dsdvo_input" type="checkbox" name="show_policy" <?php echo esc_html($show_policy); ?>/><br />
						<label><?php echo wp_kses(__("Should the Cookie Notice be displayed?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					</p>
					...
					<p class="dsdvo_options" style="display: none !important;">
						<b><?php echo wp_kses(__("Automatic approval by changing the page", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
						<input  class="dsdvo_input" type="checkbox" name="auto_accept" <?php echo esc_html($auto_accept); ?>/><br />
						<label><?php echo wp_kses(__("Should the cookie and external services be automatically enabled when the user navigates on the site?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					</p>
					

					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Service Control", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp; 
						<input  class="dsdvo_input" type="checkbox" name="show_servicecontrol" <?php echo esc_html($show_servicecontrol); ?>/><br />
						<label><?php echo wp_kses(__("Should the service control be displayed?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
						<br />
						<span class="servicecontrolwrap">
						<br />
						<b><?php echo wp_kses(__("Position Service Control", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
						...
						<br />
						<label><?php echo wp_kses(__("In which corner the service control should be displayed?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>						
						</span>
					</p>
					...
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Cookie Notice Style / Position", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>  <br />
						<select name="notice_style" id="dsgvoaio_select_style">
							<option value="1" <?php if ($notice_style == 1) { echo "selected";} ?>><?php echo wp_kses(__("#1 Sticky at the bottom of the screen", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></option>
							<option value="2" <?php if ($notice_style == 2) { echo "selected";} ?>><?php echo wp_kses(__("#2 Overlay over the entire page", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></option>
							<option value="3" <?php if ($notice_style == 3) { echo "selected";} ?>><?php echo wp_kses(__("#3 Overlay entire page + Privacy Policy *RECOMMENDED*", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></option>
							<option value="4" <?php if ($notice_style == 4) { echo "selected";} ?> disabled><?php echo wp_kses(__("Block - bottom left corner (Pro Version)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></option>
							<option value="5" <?php if ($notice_style == 5) { echo "selected";} ?> disabled><?php echo wp_kses(__("Block - bottom right corner (Pro Version)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></option>
							<option value="6" <?php if ($notice_style == 6) { echo "selected";} ?> disabled><?php echo wp_kses(__("Block - top left corner (Pro Version)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></option>
							<option value="7" <?php if ($notice_style == 7) { echo "selected";} ?> disabled><?php echo wp_kses(__("Block - top right corner (Pro Version)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></option>							
						</select><br />
						<label>
						<?php echo wp_kses(__("Select the layout/position of the cookie notice.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
						</label>
					</p>

					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Animation Time", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>  <br />
						...
						<label><?php echo wp_kses(__("Specification in milliseconds (1000 = 1 Second)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.</label>
						<br />					
					</p>	
	
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Cookie Notice Design", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>  <br />
						<select name="notice_design">
							<option value="clear" <?php if ($notice_design == "clear") { echo "selected";} ?>><?php echo wp_kses(__("Light - Grey", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></option>						
							<option value="dark" <?php if ($notice_design == "dark") { echo "selected";} ?>><?php echo wp_kses(__("Dark - Grey", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></option>							
							
						</select><br />
						<label><?php echo wp_kses(__("Select the design of the cookie notice.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					</p>

					<br />
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Button Text (Accept)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
						...
						<?php } ?>
						<?php if ($showpolylangoptions == true && array_key_exists("en", $languages) or $language == "en") { ?>
						<?php if ($showpolylangoptions == true) { ?>
						...
						<?php } ?>
						<?php if ($language == "it" or $showpolylangoptions == true && array_key_exists("it", $languages)) { ?>
						<?php if ($showpolylangoptions == true) { ?>
						...
						<b><?php echo wp_kses(__("Button Text (Personalize)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
						<?php if ($showpolylangoptions == true) { ?>
						<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<?php } ?>						
						...
						<?php } ?>		
						<?php if ($showpolylangoptions == true && array_key_exists("it", $languages) or $language == "it") { ?>
						<?php if ($showpolylangoptions == true) { ?>
						<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("Italian", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<?php } ?>	
						...
						<label>
						<?php echo wp_kses(__("Text of the button to personalize the opt in / opt out settings", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
					...
					<br />					
					<span class="dsdvo_options">
					<b><?php echo wp_kses(__("Show Reject Button", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>?</b>&nbsp;
							<input  class="dsdvo_input" type="checkbox" name="show_rejectbtn" <?php echo esc_html($show_rejectbtn); ?>/><br />
							<label><?php echo wp_kses(__("Should the button to reject the conditions be displayed?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
							<br />
							<span class="rejectbtnwrap">
								<p><?php echo wp_kses(__("Info: If no external service is activated the reject button is not displayed!", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></p>
								<br />
								<b><?php echo wp_kses(__("Button Text (Reject)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
								<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
								<?php } ?>								
								...
								<?php } ?>									
								<label>
								<?php echo wp_kses(__("Text of the button to reject the conditions", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
					...
					<br />					
					<span class="dsdvo_options">
					<b><?php echo wp_kses(__("Close Popup automatically", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>?</b>&nbsp;
							<input  class="dsdvo_input" type="checkbox" name="close_popup_auto" <?php echo esc_html($close_popup_auto); ?>/><br />
							<label><?php echo wp_kses(__("Should the personalize popup be automatically closed when all services are allowed or denied?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>					
					</span>					
					<br />					
					<span class="dsdvo_options" id="dsgvoaio_closebtn_wrap">
					<b><?php echo wp_kses(__("Show close button (X)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>?</b>&nbsp;
							<input  class="dsdvo_input" type="checkbox" name="show_closebtn" <?php echo esc_html($show_closebtn); ?>/><br />
							<label><?php echo wp_kses(__("Should the button to close the cookie notice be displayed?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>					
					</span>
					<br />						
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Cookie Lifetime", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<input class="dsdvo_input" type="text" name="cookie-time" value="<?php echo esc_html($cookie_time); ?>" /> <?php echo wp_kses(__("Day(s)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
						<label>
						<?php echo wp_kses(__("How long should the cookie of the cookie consent be stored? (<u>in days!</u>)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					...
					<p class="dsdvo_options">
					<b><?php echo wp_kses(__("DNT (Do Not Track)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
							<input  class="dsdvo_input" type="checkbox" name="use_dnt" <?php echo esc_html($use_dnt); ?>/><br />
							<label><?php echo wp_kses(__("Should the Do Not Track setting be respected? We recommend you to respect DNT because only that is GDPR compliant!", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
							<br />
					</p>
					
					<br />
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("OptIn / OptOut Layer Texts", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp; 
						<input  class="dsdvo_input" type="checkbox" name="show_layertext" <?php echo esc_html($show_layertext); ?>/><br />
						<label><?php echo wp_kses(__("Should a short privacy policy text be displayed to the user", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>?</label>
					</p>	
					...
					<span class="dsdvo_options">
						...
						<b><?php echo wp_kses(__("Conditions/Consent Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
						<a href="#" class="reset_policy_service" data-service="cookietext" title="<?php echo wp_kses(__("Cookie Notice Text neu laden", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
						</span>						
						<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
						<?php if ($showpolylangoptions == true) { ?>
						<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						...
						<?php } ?>
						<?php if ($language == "en" or $showpolylangoptions == true && array_key_exists("en", $languages)) { ?>
						<?php if ($showpolylangoptions == true) { ?>
						<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("English", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<?php } ?>
						...
						<?php } ?>						
						<label>
						<?php echo wp_kses(__("Here you can change the text of the cookie notice.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					...
					<br/>
					 ...
					 <p class="dsdvo_options" style="display: none;">
						<b><?php echo wp_kses(__("Link to page when conditions/cookies are rejected", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<input class="dsdvo_input" type="text" name="cookie_not_acceptet_url" value="<?php echo esc_url($cookie_not_acceptet_url); ?>"  /><br />
						<label>
						<?php echo wp_kses(__("If the user does not agree to the terms and conditions and clicks on \"reject\", which page should the user be redirected to? Recommendation: https://www.wko.at/service/wirtschaftsrecht-gewerberecht/EU-Datenschutz-Grundverordnung:-Auswirkungen-auf-Websites.html", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					...
					<br/>
					 <p class="dsdvo_options" style="display:none;">
						<b><?php echo wp_kses(__("Text that should appear when you have rejected the conditions", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
					...
					<b><?php echo wp_kses(__("Comfortably change ALL colors as well as ALL texts?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></b><br />
					<br />
					<?php echo wp_kses(__("No problem at all! In the Pro Version you can change really EVERYTHING comfortably via the settings.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
	
					</p>					
					</div>

					<br />
					<h2 class="dsgvoheader"><a><span class="dashicons dashicons-lock"></span><?php echo wp_kses(__("Privacy Settings <span style='font-size:13px'>(Privacy Policy, Shortcode)</span>", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></a></h2>
					<span class="dsgvooptionsinner">
					<p class="dsdvo_options">
							...
							<option disabled selected><?php echo wp_kses(__("Select Page...", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></option>
								...
								if( $pages = get_pages() ){
									foreach( $pages as $page ){
										echo '<option value="' . wp_kses($page->ID, $kses_allowed_html) . '" ' . selected( $page->ID, sanitize_text_field(get_option("dsdvo_policy_site"))) . '>' . wp_kses($page->post_title, $kses_allowed_html) . '</option>';
									}
								}
								?>
							</select><br />
						<label><?php echo wp_kses(__("Select the page where your privacy policy is located.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
		
					</p>
					<br />
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Shortcode Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><br />
						<input class="dsdvo_input" type="text" value="[dsgvo_policy]" readonly/><br />
						<label>
						<?php echo wp_kses(__("With this shortcode you can embed the privacy policy on a page", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.					
						<br />
						<b><?php echo wp_kses(__("New", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;</b>
						<?php echo wp_kses(__("Use the Gutenberg Block instead of the shortcode - to be found in the editor.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>					
						</label>
					</p>					
					<br />
					 <div class="dsdvo_options">
						<span class="dsgvoaio_lang_info scnd">
						<b><?php echo wp_kses(__("General Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
						<a href="#" class="reset_policy_service" data-service="mainpolicy" title="<?php echo wp_kses(__("Reload Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
						</span>						
						<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
						<?php if ($showpolylangoptions == true) { ?>
						<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						...
						<br />
						<label>
						<?php echo wp_kses(__("Here you can change the text of the privacy policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
						</label>					
					</div>
					<br />
					...
					<span class="policy_services dsdvo_options" style="display: block">
							<span class="dsgvoaio_lang_info scnd">					
							<b><?php echo wp_kses(__("Privacy Policy of WordPress & Plugins", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<a href="#" class="reset_policy_service" data-service="wordpress" title="<?php echo wp_kses(__("Reload WordPress Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
							</span>
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
							...
							<br />	
							<?php } ?>							
							<?php echo wp_kses(__("This text is automatically inserted at the end of the privacy policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />							
							<?php echo wp_kses(__("<b>Important:</b> Make sure that the tag [dsgvoaio_plugins] is present at the end of this text! ", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />							
						...
						jQuery(".reset_policy_service").click(function(e) {
							e.preventDefault();
							if (confirm("<?php  echo wp_kses(__("You are sure that you want perform this action?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>")) {
					...
					<span class="policy_services dsdvo_options" style="display: block">
							<b><?php echo wp_kses(__("Privacy Policy of external Services", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
							<br />
							<?php echo wp_kses(__("Below are the data protection conditions of the external services", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
							<?php echo wp_kses(__("<b>Info:</b> The data protection conditions of the services activated under point #1 are automatically loaded.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
							<?php echo wp_kses(__("This text or these texts are also automatically added to the Privacy Policy.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />							
							<br />
			
							<?php if (get_option('dsdvo_use_fbpixel') == "on") { ?> 
							<span class="dsgvoaio_lang_info scnd">
							<b><?php echo wp_kses(__("Facebook Pixel", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>						
							<a href="#" class="reset_policy_service" data-service="fbpixel" title="<?php echo wp_kses(__("Reload Facebook Pixel Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
							</span>							
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
							...
							<span class="dsgvoaio_lang_info scnd">
							<b><?php echo wp_kses(__("Facebook Like/Comments", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<a href="#" class="reset_policy_service" data-service="fb" title="<?php echo wp_kses(__("Reload Facebook Like/Comments Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
							</span>
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
							...
							<span class="dsgvoaio_lang_info scnd">
							<b><?php echo wp_kses(__("Google Analytics", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<a href="#" class="reset_policy_service" data-service="ga" title="<?php echo wp_kses(__("Reload Google Analytics Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
							</span>
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
							...
							<span class="dsgvoaio_lang_info scnd">
							<b><?php echo wp_kses(__("Google Tag Manager", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<a href="#" class="reset_policy_service" data-service="gtag" title="<?php echo wp_kses(__("Reload Google Tag Manager Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
							</span>
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
							...
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<span class="dsgvoaio_lang_info scnd">
								<b><?php echo wp_kses(__("Matomo", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
								<a href="#" class="reset_policy_service" data-service="matomo" title="<?php echo wp_kses(__("Reload Matomo Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
								</span>								
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
								<?php } ?>								
							...
							<span class="dsgvoaio_lang_info scnd">
							<b><?php echo wp_kses(__("VG Wort", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<a href="#" class="reset_policy_service" data-service="vgwort" title="<?php echo wp_kses(__("Reload VG Wort Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
							</span>							
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
							...
							<span class="dsgvoaio_lang_info scnd">
							<b><?php echo wp_kses(__("Koko Analytics", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<a href="#" class="reset_policy_service" data-service="koko" title="<?php echo wp_kses(__("Reload Koko Analytics Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
							</span>							
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
							...
							<span class="dsgvoaio_lang_info scnd">
							<b><?php echo wp_kses(__("Shareaholic", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<a href="#" class="reset_policy_service" data-service="shareaholic" title="<?php echo wp_kses(__("Reload Shareaholic Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
							</span>														
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>	
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
							...
							<span class="dsgvoaio_lang_info scnd">
							<b><?php echo wp_kses(__("LinkedIn", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<a href="#" class="reset_policy_service" data-service="linkedin" title="<?php echo wp_kses(__("Reload LinkedIn Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
							</span>																					
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>		
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
							...
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<span class="dsgvoaio_lang_info scnd"><b><?php echo wp_kses(__("Vimeo", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><a href="#" class="reset_policy_service" data-service="vimeo" title="<?php echo wp_kses(__("Reload Vimeo Privacy Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a></span>																
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
								<?php } ?>								
								<?php
							...
							<span class="dsgvoaio_lang_info scnd">
							<b><?php echo wp_kses(__("YouTube", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<a href="#" class="reset_policy_service" data-service="youtube" title="<?php echo wp_kses(__("Reload Youtube Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a>
							</span>																					
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<?php if ($showpolylangoptions == true) { ?>
								<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
					...
					<br />
					<h2 style="" class="dsgvoheader"><a><span class="dashicons dashicons-text-page"></span><?php echo wp_kses(__("Imprint", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>&nbsp;<span style="font-size:13px">(<?php echo wp_kses(__('Shortcode / Imprint', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>)</span><span class="toggle-indicator" aria-hidden="true"></span></a></h2>
					<span class="dsgvooptionsinner">	
					<div class="dsdvo_options dsgvoaio_imprint">
					<p>
						<b  class="blabel"><?php echo wp_kses(__("Legal form", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
						<select class="dsdvo_input dsdvo_legalform"  name="dsdvo_legalform[option_id]">
								...
								if( $legalform_options ){									
									foreach( $legalform_options as $legalform_option_key => $legalform_option ){
										echo '<option value="' . wp_kses($legalform_option_key, $kses_allowed_html) . '" ' . selected( $legalform_option_key, sanitize_text_field(get_option("dsdvo_legalform"))) . '>' . wp_kses($legalform_option, $kses_allowed_html) . '</option>';
									}
								}
								?>
						</select>
						<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__("Select the legal form that suits you.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>					
						</p>
					
						<span class="legal_form_first_step">
						<p>
							<b class="blabel"><?php echo wp_kses(__("Responsible person", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							...
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('This value is defined under owner data.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>
						</span>
				...
				<div class="legal_form_nocompany">	
						...
						<p>
							<b class="blabel"><?php echo wp_kses(__("E-Mail", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_email" type="text" name="legalform_email" value="<?php echo esc_html($dsgvoaiomail); ?>" readonly/>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('This value is defined under owner data.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>				
						<p>
							<b class="blabel"><?php echo wp_kses(__("Phone", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_phone" type="text" name="legalform_phone" value="<?php echo esc_html($dsgvoaiophone); ?>" readonly/>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('This value is defined under owner data.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>		
						<p>
							<b class="blabel"><?php echo wp_kses(__("Fax", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
						...
						<p>
							<b class="blabel"><?php echo wp_kses(__("Regulatory approval", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input type="radio" name="legalform_needconsens" value="yes" <?php if (isset($legalform_needconsens)) { if ($legalform_needconsens == "yes") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("Yes", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<input type="radio" name="legalform_needconsens" value="no" <?php if (isset($legalform_needconsens)) { if ($legalform_needconsens == "no") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("No", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('Is an official approval required for the exercise of your profession?', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>
						</span>
						
						<span class="supervisoryauthoritycontainer">
						<p>
							<b class="blabel"><?php echo wp_kses(__("Regulatory Authority", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							...
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('Enter the name of the supervisory authority and in the best case also the address.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>																
						</p>						
						</span>	
						
						<span class="registerentry">
						<p>
							<b class="blabel"><?php echo wp_kses(__("Register entry", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input type="radio" name="legalform_needregister" value="yes" <?php if (isset($legalform_needregister)) { if ($legalform_needregister == "yes") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("Yes", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<input type="radio" name="legalform_needregister" value="no" <?php if (isset($legalform_needregister)) { if ($legalform_needregister == "no") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("No", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('Is registration in a register required for your company?', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>						
						</span>	
						...
						<p>
						<b class="blabel"><?php echo wp_kses(__("Register", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<select class="dsdvo_input dsdvo_legalform_register"  name="legalform_register[option_id]">
									...
									if( $legalform_register_options ){										
										foreach( $legalform_register_options as $legalform_register_option_key => $legalform_register_option ){
											echo '<option value="' . wp_kses($legalform_register_option_key, $kses_allowed_html) . '" ' . selected( $legalform_register_option_key, sanitize_text_field(get_option("dsdvo_legalform_register"))) . '>' . wp_kses($legalform_register_option, $kses_allowed_html) . '</option>';
										}
									}
									?>
							</select>						
						...
						<p>
							<b class="blabel"><?php echo wp_kses(__("City", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_city" type="text" name="legalform_city" value="<?php echo wp_kses(stripslashes(esc_html($legalform_city)), $kses_allowed_html); ?>"/>
						</p>						
						<p>
							<b class="blabel"><?php echo wp_kses(__("Register number", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_registernumber" type="text" name="legalform_registernumber" value="<?php echo wp_kses(stripslashes(esc_html($legalform_registernumber)), $kses_allowed_html); ?>"/>
						</p>
						</span>
					...
					<span class="legal_form_opt2">
						<p>
						<b class="blabel"><?php echo wp_kses(__("Profession with duty to inform", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<select class="dsdvo_input dsdvo_legalform_inforule"  name="legalform_inforule[option_id]">
									...
									if( $legalform_register_options ){											
										foreach( $legalform_register_options as $legalform_register_option_key => $legalform_register_option ){
											echo '<option value="' . wp_kses($legalform_register_option_key, $kses_allowed_html) . '" ' . selected( $legalform_register_option_key, sanitize_text_field(get_option("dsdvo_legalform_inforule"))) . '>' . wp_kses($legalform_register_option, $kses_allowed_html) . '</option>';
										}
									}
									?>
							</select>						
		...
		<span class="legalform_inforulecontaineryes">
						<p>
							<b class="blabel"><?php echo wp_kses(__("Competent chamber", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_chamber" type="text" name="legalform_chamber" value="<?php echo wp_kses(stripslashes(esc_html($legalform_chamber)), $kses_allowed_html); ?>"/>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('Geben Sie hier den Namen der Kammer ein die für Ihr Unternehmen zuständig ist.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>																
						</p>	
						<p>
							<b class="blabel"><?php echo wp_kses(__("State", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<select class="dsdvo_input legalform_state"  name="legalform_state[option_id]">
									...
									if( $legalform_state_options ){											
										foreach( $legalform_state_options as $legalform_state_option_key => $legalform_state_option ){
											echo '<option value="' . wp_kses($legalform_state_option_key, $kses_allowed_html) . '" ' . selected( $legalform_state_option_key, sanitize_text_field(get_option("dsdvo_legalform_state"))) . '>' . wp_kses($legalform_state_option, $kses_allowed_html) . '</option>';
										}
									}
									?>
							</select>							</p>	
					...
					<span class="legalform_inforulecontainer">
						<p>
							<b class="blabel"><?php echo wp_kses(__("Vat-ID", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_ustid" type="text" name="legalform_ustid" value="<?php echo wp_kses(stripslashes(esc_html($legalform_ustid)), $kses_allowed_html); ?>"/>
						</p>	
						<p>
							<b class="blabel"><?php echo wp_kses(__("Economic ID", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_wid" type="text" name="legalform_wid" value="<?php echo wp_kses(stripslashes(esc_html($legalform_wid)), $kses_allowed_html); ?>"/>
						</p>							
					</span>	
					...
					<p>
							<b class="blabel"><?php echo wp_kses(__("Editorial or journalistic content", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
							<input type="radio" name="legalform_journalist" value="yes" <?php if (isset($legalform_journalist)) { if ($legalform_journalist == "yes") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("Yes", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<input type="radio" name="legalform_journalist" value="no" <?php if (isset($legalform_journalist)) { if ($legalform_journalist == "no") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("No", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('Do you publish editorial or journalistic content?', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>	
					</span>	
					...
					<span class="legal_form_jornal_container">	
						<p>
							<b class="blabel"><?php echo wp_kses(__("Responsible person", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_personname_jornalist" type="text" name="legalform_personname_jornalist" value="<?php echo wp_kses(stripslashes(esc_html($legalform_personname_jornalist)), $kses_allowed_html); ?>"/>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('This value is defined under owner data.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>					
						<p>
							<b class="blabel"><?php echo wp_kses(__("Street & house number", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_adress_jornalist" type="text" name="legalform_adress_jornalist" value="<?php echo wp_kses(stripslashes(esc_html($legalform_adress_jornalist)), $kses_allowed_html); ?>"/>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('This value is defined under owner data.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>				
						<p>
							<b class="blabel"><?php echo wp_kses(__("Zip code", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_zip_jornalist" type="text" name="legalform_zip_jornalist" value="<?php echo wp_kses(stripslashes(esc_html($legalform_zip_jornalist)), $kses_allowed_html); ?>"/>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('This value is defined under owner data.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>		
						<p>
							<b class="blabel"><?php echo wp_kses(__("City", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_city_jornalist" type="text" name="legalform_city_jornalist" value="<?php echo wp_kses(stripslashes(esc_html($legalform_city_jornalist)), $kses_allowed_html); ?>"/>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('This value is defined under owner data.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>	
						<p>
							<b class="blabel"><?php echo wp_kses(__("Country", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
							<input  class="dsdvo_input legalform_country_jornalist" type="text" name="legalform_country_jornalist" value="<?php echo wp_kses(stripslashes(esc_html($legalform_country_jornalist)), $kses_allowed_html); ?>"/>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('This value is defined under owner data.', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>	
					</span>	
					...
					<span class="">	
					<p>
							<b class="blabel"><?php echo wp_kses(__("Disclaimer", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
							<input type="radio" name="clause" value="yes" <?php if (isset($clause)) { if ($clause == "yes") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("Yes", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<input type="radio" name="clause" value="no" <?php if (isset($clause)) { if ($clause == "no") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("No", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('Should a disclaimer be inserted for contents and links?', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>	
					</span>				

					<span class="">	
					<p>
							<b class="blabel"><?php echo wp_kses(__("Copyright", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
							<input type="radio" name="copyright" value="yes" <?php if (isset($copyright)) { if ($copyright == "yes") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("Yes", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<input type="radio" name="copyright" value="no" <?php if (isset($copyright)) { if ($copyright == "no") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("No", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('Should a passage for the copyright be inserted?', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>	
					</span>							
	
					<span class="">	
					<p>
							<b class="blabel"><?php echo wp_kses(__("Own texts", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
							<input type="radio" name="owntextsimprint" value="yes" <?php if (isset($owntextsimprint)) { if ($owntextsimprint == "yes") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("Yes", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<input type="radio" name="owntextsimprint" value="no" <?php if (isset($owntextsimprint)) { if ($owntextsimprint == "no") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("No", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
							<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('Do you want to add your own texts such as an image source reference or other texts?', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
						</p>	
					</span>	
					...
					<span>	
					<p>
						<b class="blabel"><?php echo wp_kses(__("Spam Protection (Email Adress)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
						<input type="radio" name="spamemail" value="yes" <?php if (isset($spamemail)) { if ($spamemail == "yes") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("Yes", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<input type="radio" name="spamemail" value="no" <?php if (isset($spamemail)) { if ($spamemail == "no") { echo "checked='checked'"; }} ?>>&nbsp;<?php echo wp_kses(__("No", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
						<span  class="dsgvoaio_tooltip tooltip" title="<?php echo wp_kses(__('Should the email address be displayed as graphic instead of text?', "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>" ><span class="dashicons dashicons-editor-help"></span></span>										
					</p>	
					</span>						
				</div>
					</span>							
					<br />
					<h2 class="dsgvoheader"><a><span class="dashicons dashicons-admin-comments"></span><?php echo wp_kses(__("Privacy settings <span style='font-size:13px'>(Blog comments)</span>", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></a></h2>
					<span class="dsgvooptionsinner">
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Accept Blog Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> 
						<input  class="dsdvo_input" type="checkbox" name="blog_agb" <?php echo esc_html($blog_agb_selected); ?>/><br />
						<label><?php echo wp_kses(__("Should the privacy policy be accepted when creating a blog comment?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					</p>
					<br />
					<div class="dsgvoaio_blog_policy_wrap">
					<span class="dsdvo_options">
						<b><?php echo wp_kses(__("Text next to the checkbox for acceptance of the privacy policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>  <br />
						<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
						<?php if ($showpolylangoptions == true) { ?>
						<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<?php } ?>	
						...
						<?php } ?>						
						<label><?php echo wp_kses(__("Text to display next to the checkbox of the blog comment form.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					</span>
					<br />
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Text if the conditions were not accepted", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>  <br />
						<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
						<?php if ($showpolylangoptions == true) { ?>
						<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<?php } ?>						
						...
						<?php } ?>							
						<label><?php echo wp_kses(__("Text to appear if the privacy policy has not been accepted - no HTML allowed!", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					</p>
					</div>
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Delete existing IP addresses", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><br />
						<?php
						$nonce = wp_create_nonce( 'dsgvo-delete-ip-adresses' );
						?>
						<a href="#" class='button button-primary dsgvo_delete_ip_adresses' data-nonce="<?php echo esc_attr($nonce); ?>"><?php echo wp_kses(__("Delete IP addresses", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></a><br />
						 <label>
						<?php echo wp_kses(__("Here you can delete the existing IP addresses of already posted comments.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
						<?php echo wp_kses(__("By clicking on the button Delete IP Addresses all IP addresses are deleted immediately.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
						</label>
					</p>	
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Remove IP addresses automatically", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
						<input  class="dsdvo_input" type="checkbox" name="dsgvo_remove_ipaddr_auto" <?php echo esc_html($dsgvo_remove_ipaddr_auto); ?>/><br />
						 <label>
						<?php echo wp_kses(__("If enabled, the IP addresses of the comments will be anonymized immediately upon sending before saving..", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
						</label>
					</p>
					</span>	
					...
					<h2 class="dsgvoheader"><a><span class="dashicons dashicons-lock"></span><?php echo wp_kses(__("Texts OptIn OptOut Layer", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>&nbsp;<span class="toggle-indicator" aria-hidden="true"></span></a></h2>
					<div class="dsgvooptionsinner layertextsdsgvoaio">
					<?php $count = 0; ?>
					<?php if (get_option('dsdvo_show_layertext') != "on") { ?>
						<span class="dsgvoaio_lang_info scnd"><span class="dashicons dashicons-info"></span><?php echo wp_kses(__("You have to activate the option Cookie Notice -> Show OptIn / OutOut Layer texts. Only then the following texts will be used.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></span>																
					<?php } ?>
					
							<?php if (get_option('dsdvo_use_vimeo') == "on") { ?>
							<?php $count = $count+1; ?>							
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<span class="dsgvoaio_lang_info scnd"><b><?php echo wp_kses(__("Vimeo", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><a href="#" class="load_layer_policy" data-service="vimeo" title="<?php echo wp_kses(__("Edit Vimeo Layer Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-edit"></span></a><a href="#" class="reset_layertext_service" data-service="vimeo" title="<?php echo wp_kses(__("Reload Vimeo Layer Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a></span>																
							...
							<?php $count = $count+1; ?>							
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<span class="dsgvoaio_lang_info scnd"><b><?php echo wp_kses(__("Youtube", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><a href="#" class="load_layer_policy" data-service="youtube" title="<?php echo wp_kses(__("Edit Youtube Layer Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-edit"></span></a><a href="#" class="reset_layertext_service" data-service="youtube" title="<?php echo wp_kses(__("Reload Youtube Layer Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a></span>																
							...
							<?php $count = $count+1; ?>							
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<span class="dsgvoaio_lang_info scnd"><b><?php echo wp_kses(__("Linkedin", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><a href="#" class="load_layer_policy" data-service="linkedin" title="<?php echo wp_kses(__("Edit Linkedin Layer Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-edit"></span></a><a href="#" class="reset_layertext_service" data-service="linkedin" title="<?php echo wp_kses(__("Reload Linkedin Layer Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a></span>																
							...
							<?php $count = $count+1; ?>							
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<span class="dsgvoaio_lang_info scnd"><b><?php echo wp_kses(__("VG Wort", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><a href="#" class="load_layer_policy" data-service="vgwort" title="<?php echo wp_kses(__("Edit VG Wort Layer Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-edit"></span></a><a href="#" class="reset_layertext_service" data-service="vgwort" title="<?php echo wp_kses(__("Reload VG Wort Layer Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a></span>																
							...
							<?php $count = $count+1; ?>							
							<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
								<span class="dsgvoaio_lang_info scnd"><b><?php echo wp_kses(__("Shareaholic", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b><a href="#" class="load_layer_policy" data-service="shareaholic" title="<?php echo wp_kses(__("Edit Shareaholic Layer Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-edit"></span></a><a href="#" class="reset_layertext_service" data-service="shareaholic" title="<?php echo wp_kses(__("Reload Shareaholic Layer Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>"><span class="dashicons dashicons-image-rotate"></span></a></span>																
					...
					<h2 class="dsgvoheader"><a><?php echo wp_kses(__("#5 Impressum <span style='font-size:13px'>(Shortcode and Text)</span>", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></a></h2>
					<span class="dsgvooptionsinner">
					<p class="dsdvo_options">
					<b><?php echo wp_kses(__("Does this website have an online store", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>?</b>&nbsp;
					<input  class="dsdvo_input" type="checkbox" name="is_online_shop" <?php echo esc_html($is_online_shop); ?>/><br />
					
					<br />

					<b class="blabel"><?php echo wp_kses(__("Operator of this website", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>
					<input  class="dsdvo_input companyformat" type="text" name="companyformat" value="<?php echo esc_html($companyformat); ?>"/><br />
					<label><?php echo wp_kses(__("Operator of this website", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br /></label>					
					</p>					
					</span>
					...
					<h2 class="dsgvoheader"><a><span class="dashicons dashicons-trash"></span><?php echo wp_kses(__("Settings Delete Account <span style='font-size:13px'>(Shortcode for removal)</span>", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></a></h2>
					<span class="dsgvooptionsinner">
					<?php if ( get_option( 'users_can_register' ) == 0 ) { ?>
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("INFO", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;</b>
						<?php echo wp_kses(__("The user login is disabled in the WordPress settings. If you do not allow registration you can ignore this point", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />					
					</p>
					<?php } ?>					
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Deletion of the user account", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> (<?php echo wp_kses(__("Right to be forgotten", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>) <br />
						<input class="dsdvo_input"  type="text"  value="[dsgvo_user_remove_form]" readonly/><br />
						<label>
						<?php echo wp_kses(__("The shortcode for the form to delete the user account including all data.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
						<?php echo wp_kses(__("Paste the shortcode on the page where you want to display the form.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
						<br />
						<b><?php echo wp_kses(__("New", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;</b>
						<?php echo wp_kses(__("Use the Gutenberg Block instead of the shortcode - to be found in the editor.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>										
						</label>
					...
					<br />
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Account deletion page", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>  <br />
						<select class="dsdvo_input"  name="dsdvo_delete_account_page[page_id]">
								<?php
								if( $pages = get_pages() ){
									foreach( $pages as $page ){
										echo '<option value="' . wp_kses($page->ID, $kses_allowed_html) . '" ' . selected( $page->ID, sanitize_text_field(get_option("dsdvo_delete_account_page"))) . '>' . wp_kses($page->post_title, $kses_allowed_html) . '</option>';
									}
								}
								?>
							</select><br />
						<label><?php echo wp_kses(__("Select the page on which the shortcode [dsgvo_user_remove_form] is located.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					...
					<br />
					<h2 class="dsgvoheader"><a><span class="dashicons dashicons-visibility"></span><?php echo wp_kses(__("Settings User Data Extract <span style='font-size:13px'>(Data Extract Shortcode, PDF Settings)</span>", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></a></h2>
					<span class="dsgvooptionsinner">
					<?php if ( get_option( 'users_can_register' ) == 0 ) { ?>
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("INFO", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;</b>
					...
					<?php } ?>					
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Data extract", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> (<?php echo wp_kses(__("Right of information", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>) <br />
						<input class="dsdvo_input"  type="text"  value="[dsgvo_show_user_data]" readonly/><br />
						 <label>
						<?php echo wp_kses(__("The shortcode creates an output of all stored data about a user.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
						<?php echo wp_kses(__("Der Shortcode erzeugt eine Ausgabe aller gespeicherten Daten über einen Benutzer.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
						<br />
						<b><?php echo wp_kses(__("New", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;</b>
						<?php echo wp_kses(__("Use the Gutenberg Block instead of the shortcode - to be found in the editor.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
						</label>
					...
					<br />
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("PDF output of the data", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> (<?php echo wp_kses(__("Information Text", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>) <br />
						<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
						<?php if ($showpolylangoptions == true) { ?>
						<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<?php } ?>							
						...
						<?php } ?>						
						<label>
						<?php echo wp_kses(__("Enter here the text that you want to appear in the PDF below", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
						<?php echo wp_kses(__("If you do not want to display any text, leave this field empty", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.
						</label>
					</p>
					<br />
					</span>
					...
					<br />
					<h2 class="dsgvoheader"><a><span class="dashicons dashicons-admin-generic"></span><?php echo wp_kses(__("Settings Opt in & Out <span style='font-size:13px'>(Shortcode for the output of the current status of the services)</span>", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></a></h2>
					<span class="dsgvooptionsinner">
					<p class="dsdvo_options">
					<b><?php echo wp_kses(__("Opt in & Out Settings", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> (<?php echo wp_kses(__("Important", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>) <br />
					<input class="dsdvo_input"  type="text"  value="[dsgvo_service_control]" readonly/><br />
					<label>
					<?php echo wp_kses(__("The shortcode allows the user to see which opt in / external services he has allowed and which not.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<?php echo wp_kses(__("The user can also change the rights which external services are allowed or not.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
					<?php echo wp_kses(__("Insert the shortcode on the page where you want to display the data.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
					<br />
					<b><?php echo wp_kses(__("New", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;</b>
					<?php echo wp_kses(__("Use the Gutenberg Block instead of the shortcode - to be found in the editor.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>					
					</label>
					...
					<h2 class="dsgvoheader"><a><span class="dashicons dashicons-admin-links"></span><?php echo wp_kses(__("Outgoing Links <span style='font-size:13px'>(Notice)</span>", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></a></h2>
					<span class="dsgvooptionsinner">
					<p class="dsdvo_options">
						<b><?php echo wp_kses(__("Enable Notice", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>&nbsp;
						<input  class="dsdvo_input" type="checkbox" name="show_outgoing_notice" <?php echo esc_html($show_outgoing_notice); ?>/><br />
						<label><?php echo wp_kses(__("Should a notice appear when clicking on outgoing links?", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					</p>
					...
					<span class="dsdvo_options">
						<b><?php echo wp_kses(__("Text of the notice for the outgoing links", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b>  <br />
						<?php if ($language == "de" or $showpolylangoptions == true && array_key_exists("de", $languages)) { ?>
						<?php if ($showpolylangoptions == true) { ?>
						<b class="dsgvoaio_lang_info"><?php echo wp_kses(__("German", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b> <br />
						<?php } ?>	
						...
						<?php } ?>						
						<label><?php echo wp_kses(__("Text to be displayed to the user as information that he is now leaving the website.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></label>
					</span>					
					</span>	
					<br />
					</span>			
					<h2 class="dsgvoheader"><a><span class="dashicons dashicons-google"></span><?php echo wp_kses(__("Google Fonts <span style='font-size:13px'>(Check whether Google fonts are used)</span>", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></a></h2>
					<span class="dsgvooptionsinner">
					...
					$allfonts = get_option('dsgvoaio_gfonts_all_fonts');
						if (isset($allfonts) && isset($allfonts[1])) {
							?>
							<span style="color:red;"><b><?php echo wp_kses(__("Attention", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></b></span><br />
							<span><?php echo wp_kses(__("Google fonts were found. Currently, these fonts are integrated into your website via Google's servers, which is <u>not GDPR compliant</u>.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></span><br />
							<span><?php echo wp_kses(__("The <a href='https://dsgvo-for-wp.com/plugin/dsgvo-all-in-one-pro-version/' target='blank'>Pro Version</a> of DSGVO All In One offers the function of automatically downloading all found Google fonts to your server and removing the integrations via the Google servers.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></span><br />							
							<br />
							<span class="dsgvoaio_green"><b><?php echo wp_kses(__("The following fonts were found:", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:</b></span><br />
						...
						<?php } else { ?>
							<span style="color:green;"><?php echo wp_kses(__("Perfect, no Google Fonts found", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></span><br />
							<?php echo wp_kses(__("No fonts were found from Google", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>.<br />
							<?php echo wp_kses(__("There is currently no need for action on your part.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
						<?php } ?>
					</p>
					<br />
					</span>										
...
<div id="dsdvo_right">
	...
	<div class="dsgvoaio_active_services">				
		...
		<h2><?php echo wp_kses(__("Cookie Notice", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></h2>
		<ul>
			<li>
				<?php echo wp_kses(__("Cookie Notice enabled", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:
			...
			<li>
				<?php echo wp_kses(__("Service Control enabled", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:
			...
			<li>
				<?php		
					...
					if (isset($multiactiv) && $multiactiv == true) {
						...
						<?php echo wp_kses(__("Multilingualism active", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>&nbsp;<span style="color:green"><?php echo esc_html($multiplugin); ?></span><br />
...
<br />
	...
	?>
		...
		<div>
			<h2><?php echo wp_kses(__("Shortcode Check", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></h2>
				<ul>
					<?php if (isset($dsgvo_policy_status)) { ?>
						<li><?php echo wp_kses(__("Privacy Policy", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;<?php echo wp_kses($dsgvo_policy_status, $kses_allowed_html); ?></li>
					<?php } ?>
					<?php if (isset($dsgvo_imprint_status)) { ?>
						<li><?php echo wp_kses(__("Imprint", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;<?php echo wp_kses($dsgvo_imprint_status, $kses_allowed_html); ?></li>
					<?php } ?>		
					<?php if ( get_option( 'users_can_register' ) == 1 ) { ?>
					<?php if (isset($dsgvo_user_remove_form_status)) { ?>
						<li><?php echo wp_kses(__("User data deletion", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;<?php echo wp_kses($dsgvo_user_remove_form_status, $kses_allowed_html); ?></li>
					<?php } ?>
					<?php if (isset($dsgvo_show_user_data_status)) { ?>
						<li><?php echo wp_kses(__("User data overview", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;<?php echo wp_kses($dsgvo_show_user_data_status, $kses_allowed_html); ?></li>
					<?php } ?>	
					<?php } ?>
					<?php if (isset($dsgvo_opt_in_and_out_status)) { ?>
						<li><?php echo wp_kses(__("Opt in & Out overview", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;<?php echo wp_kses($dsgvo_opt_in_and_out_status, $kses_allowed_html); ?></li>
					<?php } ?>
				</ul>
				<?php if ( get_option( 'users_can_register' ) == 1 ) { ?>
				<p <?php if ($shortcodecount > 5 or $shortcodecount == 5) { echo "style='display:none;'"; } ?>><u><?php echo wp_kses(__("Insert all shortcodes on pages", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>!</u></p>
				<?php } else { ?>
				<p <?php if ($shortcodecount > 3 or $shortcodecount == 3) { echo "style='display:none;'"; } ?>><u><?php echo wp_kses(__("Insert all shortcodes on pages", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>!</u></p>
				<?php } ?>
		</div>
...
<br />
	...
	<div class="dsgvoaio_active_services">				
		...
		<h2><?php echo wp_kses(__("Activated Services", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></h2>
			<?php $service_count = 0; ?>
			<ul>
			<?php if (get_option("dsdvo_use_youtube") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("YouTube", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>
			<?php if (get_option("dsdvo_use_vimeo") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("Vimeo", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>				
			<?php if (get_option("dsdvo_use_fbpixel") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("Facebook Pixel", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>
			<?php if (get_option("dsdvo_use_ga") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("Google Analytics", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>
			<?php if (get_option("dsdvo_use_piwik") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("Matomo (Piwik)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>
			<?php if (get_option("dsdvo_use_gtagmanager") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("Google Tag Manager", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>
			<?php if (get_option("dsdvo_use_vgwort") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("VG Wort", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>
			<?php if (get_option("dsdvo_use_koko") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("Koko Analytics", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>			
			<?php if (get_option("dsdvo_use_gatag") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("Google Analytics (gtag.js)", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>
			<?php if (get_option("dsdvo_use_facebookcomments") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("Facebook Comments", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>
			<?php if (get_option("dsdvo_use_facebooklike") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("Facebook Like/Share", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>
			<?php if (get_option("dsdvo_use_twitter") == "on") { ?>
			...
			<?php } ?>
			<?php if (get_option("dsdvo_use_shareaholic") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("Shareaholic", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>
			<?php if (get_option("dsdvo_use_linkedin") == "on") { ?>
				<li><span class="dashicons dashicons-plus"></span>&nbsp;<?php echo wp_kses(__("LinkedIn", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
			<?php $service_count++; ?>
			<?php } ?>
			</ul>
			<?php if ($service_count == 0) { ?>
				<?php echo wp_kses(__("No Services activated yet", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
			<?php } ?>
		</div>
<br />

	<h2><?php echo wp_kses(__("Systemcheck", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></h2>
		<ul>
			<li><?php echo wp_kses(__("max_input_vars", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;<?php echo wp_kses(ini_get("max_input_vars"), $kses_allowed_html); ?>&nbsp;(<?php echo wp_kses(__("4000", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>)</li>
			<li><?php echo wp_kses(__("max_execution_time", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;<?php echo wp_kses(ini_get("max_execution_time"), $kses_allowed_html); ?>&nbsp;(<?php echo wp_kses(__("300", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>)</li>
			<li><?php echo wp_kses(__("max_input_time", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>:&nbsp;<?php echo wp_kses(ini_get("max_input_time"), $kses_allowed_html); ?>&nbsp;(<?php echo wp_kses(__("60", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>)</li>
			<li><?php echo wp_kses(__("The values in brackets are the recommended values. If these values are not reached <u>can</u> problems may occur when saving the settings.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></li>
		</ul>
		
<br />

	<h2><?php echo wp_kses(__("Many more Options....", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></h2>
	...
	if ($language == "de") {
	?>
	<img src="<?php echo wp_kses(plugins_url( '../assets/img/pro.png', dirname(__FILE__) ), $kses_allowed_html) ?>"/>
	<?php } else { ?>
	<img src="<?php echo wp_kses(plugins_url( '../assets/img/pro_en.png', dirname(__FILE__) ), $kses_allowed_html) ?>"/>
	<?php } ?>
	</a>
	
	
	<h2><?php echo wp_kses(__("Rate this Plugin....", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></h2>
	<?php $plugin_dir_path = dirname(__FILE__); ?>
	...
	<span class="starscontent">
		...
		<p><?php echo wp_kses(__("If you like the plugin, we would be very pleased about an review.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br />
		<?php echo wp_kses(__("This does not take long and so you can appreciate our work.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?><br /><br />
		<?php echo wp_kses(__("Before you give a bad review, describe your problems in the Support Forum.", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?>
		</p>
		<span class="rateplugin button button-primary"><?php echo wp_kses(__("Rate Plugin", "dsgvo-all-in-one-for-wp"), $kses_allowed_html); ?></span>
	</span>
	</a>
</div>
